...
bazel-bin/im2txt/run_attack_BATCH_search_C  --use_keywords=False --iters=80 --checkpoint_path=${CHECKPOINT_PATH}  --caption_file=${CAPTION_FILE} --vocab_file=${VOCAB_FILE}   --input_files=${IMAGE_FILE} --image_directory=${IMAGE_DIRECTORY} --targeted=False --use_logits=True --exp_num=$2 --norm="l2" --offset=$1 --result_directory="/debug_folder/" -seed=8 --C=1 2>&1|tee debug_folder/log_$1.txt
source deactivate
